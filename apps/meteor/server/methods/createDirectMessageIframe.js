...
export function createDirectMessageIframe(usernames, userId, excludeSelf = false) {
	...
	if (!userId) {
		throw new Meteor.Error('error-invalid-user', 'Invalid user', {
			method: 'createDirectMessageIframe',
		});
	}
...
Meteor.methods({
	createDirectMessageIframe(...usernames) {
		return createDirectMessageIframe(usernames, Meteor.userId());
	},
});

RateLimiter.limitMethod('createDirectMessageIframe', 10, 60000, {
	userId(userId) {
