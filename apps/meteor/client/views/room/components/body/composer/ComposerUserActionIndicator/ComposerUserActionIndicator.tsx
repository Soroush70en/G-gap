...
import React, { useCallback, Fragment, useEffect, useLayoutEffect } from 'react';
...
const ComposerUserActionIndicator = ({ rid, tmid }: { rid: string; tmid?: string }): ReactElement => {
	document.querySelector<HTMLElement>('[data-qa-id="MessageList"]') ||
		document.querySelector<HTMLElement>('.messages-container .wrapper') ||
		document.querySelector<HTMLElement>('.rcx-main-content .rcx-box--scroll') ||
		document.querySelector<HTMLElement>('.rcx-box--scroll');

	const isNearBottom = (el: HTMLElement, threshold = 96) => el.scrollHeight - el.scrollTop - el.clientHeight <= threshold;
	...
	useEffect(() => {
		...
		root.style.setProperty('--ggap-typing-offset', isTyping ? '2.5rem' : '0');
		return () => root.style.removeProperty('--ggap-typing-offset');
	}, [isTyping]);
	...
	return isTyping ? (
		<Box className='ggap-typing-wrap' role='status' aria-live='polite' data-own='false'>
			<Box className='ggap-typing-bubble'>
				...
				<svg className='ggap-typing-tail' viewBox='0 0 14 18' width={14} height={18} preserveAspectRatio='xMidYMid meet' aria-hidden='true'>
				...
				{/* سه نقطه */}
				<span className='ggap-typing-dots'>
					<i></i>
					<i></i>
					<i></i>
				</span>
